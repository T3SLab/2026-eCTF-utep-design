 ...
 * @param nonce A pointer to a 12-byte unique value (must be unique per encryption)
 * @param ciphertext A pointer to a buffer of length len where the resulting
 *          ciphertext will be written to
 * @param auth_tag A pointer to a 16-byte buffer where the authentication
 *          tag will be written to
 *
 *
 * @return 0 on success, -1 on bad length, other non-zero for other error
 */
// modified to include the nonce and the auth_tag
