...
def secrets_to_c_header(
    ...
    # It is generated by gen_secrets alongside global.secrets and must be bind-mounted
    # as a writable file at /secrets/host_keys.json for every firmware build.
    ...
    if not devices:
        raise RuntimeError(
            f"{host_keys_path} is missing or empty. "
            "Ensure gen_secrets has been run and host_keys.json is bind-mounted "
            "as a writable file (not inside a read-only directory mount)."
        )
