...
def secrets_to_c_header(
    ...
    # Retrieve the specific ID for this HSM build (0-7)
    # Ensure your build system passes HSM_ID as an environment variable
    HSM_ID = int(os.environ.get("HSM_ID", 0))

    # Extract this device's private identity [cite: 78, 81]
    my_priv_bytes = bytes.fromhex(data["hsm_devices"][HSM_ID]["private_key"])
    
    # Peer table: Public Keys used for signature verification [cite: 80, 82, 167]
